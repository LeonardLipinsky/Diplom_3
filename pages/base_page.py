...
class BasePage:
    ...
    def click_element(self, locator):
        target = self.check_element_clickable(locator)
        click = ActionChains(self.driver)
        click.move_to_element(target).click().perform()

    @allure.step('Клик на элемент с паузой')
    def click_on_element_with_pause(self, locator):
        target = self.check_element_clickable(locator)
        click = ActionChains(self.driver)
        click.move_to_element(target).pause(5).click().perform()
